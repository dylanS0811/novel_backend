...
public class BookLikePO implements Serializable {
    ...
    public BookLikePO(LikeBookmarkKey id, BookPO book, UserPO user, LocalDateTime createdAt) {
        this.id = id;
        this.book = book;
        this.user = user;
    ...
    public static BookLikePO of(BookPO book, UserPO user, LocalDateTime now) {
        var key = new LikeBookmarkKey(book.getId(), user.getId());
        return new BookLikePO(key, book, user, now);
    }
